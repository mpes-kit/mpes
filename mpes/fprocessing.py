...
def SortNamesBy(namelist, pattern):
    ...
    seqnum = np.array([re.search(pattern, namelist[i]).group(1) for i in range(len(namelist))])
...
def rot2d(th, angle_unit):
    ...
    if angle_unit == 'deg':
        thr = np.deg2rad(th)
        return np.array([[np.cos(thr), -np.sin(thr)], [np.sin(thr), np.cos(thr)]])
...
def parsenum(NumberPattern, strings, CollectorList, linenumber, offset=0, Range='all'):
    """
    Number parser for reading calibration file
    """
    
    # Specify Range as 'all' to take all numbers, specify number limits to pick certain number
...
def readLensModeParameters(calibfiledir, lensmode='WideAngleMode'):
    ...
    if lensmode == 'WideAngleMode':
        ...
        with open(calibfiledir,'r') as fc:
            ...
            fc.seek(0)
            # Scan through calibration file, find and append line indices (lind) to specific lens settings
        ...
        for linum in LensModeDefaults:
            ...
            aRange = parsenum(numpattern, calib, aRange, linenumber=linum, offset=2, Range='all')
            # Collect the eShift
            eShift = parsenum(numpattern, calib, eShift, linenumber=linum, offset=3, Range='all')
        ...
        for linum in LensParamLines:
            ...
            rr = parsenum(numpattern, calib, rr, linenumber=linum, offset=0, Range='all')
            # Collect the aInner coefficient
            aInner = parsenum(numpattern, calib, aInner, linenumber=linum, offset=1, Range='all')
            # Collect Da1 coefficients
            Da1 = parsenum(numpattern, calib, Da1, linenumber=linum, offset=2, Range='1:4')
            # Collect Da3 coefficients
            Da3 = parsenum(numpattern, calib, Da3, linenumber=linum, offset=3, Range='1:4')
            # Collect Da5 coefficients
            Da5 = parsenum(numpattern, calib, Da5, linenumber=linum, offset=4, Range='1:4')
            # Collect Da7 coefficients
            Da7 = parsenum(numpattern, calib, Da7, linenumber=linum, offset=5, Range='1:4')
        
        aRange, eShift, rr, aInner = list(map(lambda x: np.asarray(x, dtype = 'float').ravel(), [aRange, eShift, rr, aInner]))
        Da1, Da3, Da5, Da7 =  list(map(lambda x: np.asarray(x, dtype = 'float'), [Da1, Da3, Da5, Da7]))
...
def MCP_Position_mm(Ek, Ang, aInner, Da):
    ...
    if abs(Ang) <= aInner:
        ...
        dA = abs(Ang) - aInner
        return np.sign(Ang)*(zInner(Ek, aInner, Da) + dA*zInner_Diff(Ek, aInner, Da))
...
def slice3d(datamat, xaxis, yaxis, zaxis, xmin, xmax, ymin, ymax, zmin, zmax):
    ...
    return slicedmat, xaxis[lims[0]:lims[1]], yaxis[lims[2]:lims[3]], zaxis[lims[4]:lims[5]]
...
def fftfilter2d(datamat):
    ...
    # Generate intermediate positions and append to peak center coordinates array
    ...
    for currpair in range(len(xy)):
        xc, yc = xy[currpair]
        zm += np.exp(-((x-xc)**2)/(2*sx**2) - ((y-yc)**2)/(2*sy**2))
